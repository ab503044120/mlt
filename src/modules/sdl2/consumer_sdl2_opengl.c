/*
 * consumer_sdl_opengl.c -- A Simple DirectMedia Layer consumer
...
static mlt_filter glsl_manager = NULL;
/** This classes definition.
*/

typedef struct consumer_sdl_opengl_s *consumer_sdl_opengl;

struct consumer_sdl_opengl_s {
...
static int setup_sdl_video(consumer_sdl_opengl self);

/** This is what will be called by the factory - anything can be passed in
	via the argument, but keep it simple.
*/

mlt_consumer consumer_sdl2_opengl_init(mlt_profile profile, mlt_service_type type, const char *id, char *arg) {
    // Create the consumer object
    consumer_sdl_opengl self = calloc(1, sizeof(struct consumer_sdl_opengl_s));
...
int consumer_start(mlt_consumer parent) {
    consumer_sdl_opengl self = parent->child;
...
static void sdl_fill_audio(void *udata, uint8_t *stream, int len) {
    consumer_sdl_opengl self = udata;
...
static int consumer_play_audio(consumer_sdl_opengl self, mlt_frame frame, int init_audio, int *duration) {
    ...
    int samples = mlt_audio_calculate_frame_samples(mlt_properties_get_double(self->properties, "fps"), frequency,
                                                    counter++);
    int16_t *pcm;
...
static int consumer_play_video(consumer_sdl_opengl self, mlt_frame frame) {
...
static int initGL(consumer_sdl_opengl self) {
    int error = 0;
    pthread_mutex_lock(&mlt_sdl_mutex);
    self->sdl_renderer = SDL_GL_CreateContext(self->sdl_window);
    if (!self->sdl_renderer) {
        mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "Failed to create SDL GLContext: %s\n", SDL_GetError());
        error = -1;
        goto end;
    }
    if (!gladLoadGL((GLADloadfunc) SDL_GL_GetProcAddress)) {
        mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "Failed to initialize GLAD\n");
        error = -1;
        goto end;
    }
    ...
    if (!success) {
        ...
        mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "ERROR::SHADER::VERTEX::COMPILATION_FAILED %s\n", infoLog);
        error = -1;
        goto end;
    }
    // fragment shader
    ...
    if (!success) {
        ...
        mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED %s\n", infoLog);
        error = -1;
        goto end;
    }
    // link shaders
    self->shaderProgram = glCreateProgram(); // shaderProgram 是多个着色器合并之后并最终链接完成的版本
    glAttachShader(self->shaderProgram, vertexShader); // 附加
    ...
    if (!success) {
        ...
        mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "ERROR::SHADER::PROGRAM::LINKING_FAILED %s\n", infoLog);
        error = -1;
        goto end;
    }
